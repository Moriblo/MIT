...
# Modelo atualmente utilizado pelo Mo
...
def call_groq(question):
    ...
    try:
        ...
        if "error" in data:
            return f"Erro Groq: {data['error'].get('message', 'Erro desconhecido')}"

        if not response.ok:
            return f"Erro Groq: HTTP {response.status_code}"
        ...
        return "Erro: a Groq demorou mais do que o tempo limite para responder."

    except requests.exceptions.RequestException as e:
        return f"Erro ao consultar Groq: {str(e)}"

    except (KeyError, IndexError, TypeError) as e:
        return f"Erro ao processar a resposta da Groq: {str(e)}"
